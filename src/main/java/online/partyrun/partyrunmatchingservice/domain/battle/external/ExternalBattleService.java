...
public class ExternalBattleService implements BattleService {
    ...
    public ExternalBattleService(@Value("${external.battle.url}") String battleUrl, JwtGenerator jwtGenerator) {
    ...
    public Mono<String> create(List<String> memberIds, int distance) {
        log.info("EBS 호출");
        final String systemToken = jwtGenerator.generate("MATCHING_SERVICE", Set.of("ROLE_SYSTEM")).accessToken();
        return battleClient.post().uri("/api/battles")
