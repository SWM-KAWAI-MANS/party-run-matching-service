...
/**
 * waiting 관련 service를 제공합니다.
 * {@link ServerSentEventHandler}를 이용하여 SSE 통신 register 및 subscribe을 지원합니다.
 * 또한, 대기열이 조건에 만족하면 match로 연계하도록 구현하였습니다. 현재는 대기열에 3명을 정원으로 설정하였습니다.
 *
...
public class WaitingService {
    ...
    public Flux<WaitingEventResponse> subscribe(Mono<String> runner) {
        return runner.map(id -> waitingEventHandler.connect(id)
    ...
    /**
     * Queue에 새롭게 저장합니다. queue에 정해진 값 이상이 되면 match 생성 로직을 수행합니다.
     * match 생성 로직을 수행한 이후, 연결중인 사용자 sink를 complete 합니다.
     *
