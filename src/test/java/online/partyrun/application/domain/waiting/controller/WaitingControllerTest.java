...
@WebFluxTest(controllers = WaitingController.class, excludeFilters = {
...
class WaitingControllerTest {
    ...
    @BeforeEach
    public void setUp() {
        this.webTestClient = WebTestClient.bindToController(new WaitingController(waitingService))
                .configureClient()
    ...
    void postWaiting() {
        webTestClient.post().uri("/waiting")
                .body(Mono.just(new CreateWaitingRequest(RunningDistance.M1000)), CreateWaitingRequest.class)
    ...
    void getWaiting() {
        ...
        webTestClient.get().uri("/waiting/event")
                .exchange()
                .expectStatus().isOk();
    }
}