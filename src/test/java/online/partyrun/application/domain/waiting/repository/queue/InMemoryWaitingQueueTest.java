...
class InMemoryWaitingQueueTest {
    ...
    public void clear() {
        Arrays.stream(RunningDistance.values())
                .forEach(rd -> queue.findTop(rd, queue.size(rd)));
    }

    @Nested
    @DisplayNameGeneration(DisplayNameGenerator.ReplaceUnderscores.class)
    class WaitingRunner가_주어지면 {
        List<WaitingRunner> m1000Runners = List.of(
        ...
        List<WaitingRunner> m3000Runners = List.of(
        ...
        List<WaitingRunner> m5000Runners = List.of(
        ...
        void saveAndCheckSize() {
            ...
            assertAll(
                    () -> assertThat(queue.size(RunningDistance.M1000)).isEqualTo(m1000Runners.size()),
                    () -> assertThat(queue.size(RunningDistance.M3000)).isEqualTo(m3000Runners.size()),
                    () -> assertThat(queue.size(RunningDistance.M5000)).isEqualTo(m5000Runners.size())
            );
        }
